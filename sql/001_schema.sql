...
CREATE OR REPLACE FUNCTION gen_uid(prefix TEXT) RETURNS TEXT AS $$ BEGIN RETURN prefix || '_' || upper(substr(encode(gen_random_bytes(4), 'hex'), 1, 6));
END;
$$ LANGUAGE plpgsql;
...
CREATE TRIGGER trg_projects_updated BEFORE
UPDATE ON projects FOR EACH ROW EXECUTE FUNCTION set_updated_at();
-- ASSETS
CREATE TABLE assets (
  id SERIAL PRIMARY KEY,
...
CREATE TRIGGER trg_assets_updated BEFORE
UPDATE ON assets FOR EACH ROW EXECUTE FUNCTION set_updated_at();
-- SHOTS
CREATE TABLE shots (
  id SERIAL PRIMARY KEY,
...
CREATE TRIGGER trg_shots_updated BEFORE
UPDATE ON shots FOR EACH ROW EXECUTE FUNCTION set_updated_at();
-- TASKS
CREATE TABLE tasks (
  id SERIAL PRIMARY KEY,
...
CREATE TRIGGER trg_tasks_updated BEFORE
...
CREATE TRIGGER trg_versions_updated BEFORE
UPDATE ON versions FOR EACH ROW EXECUTE FUNCTION set_updated_at();
-- PUBLISHES
...
CREATE TRIGGER trg_publishes_updated BEFORE
UPDATE ON publishes FOR EACH ROW EXECUTE FUNCTION set_updated_at();
-- RENDER JOBS
...
CREATE TRIGGER trg_render_jobs_updated BEFORE
UPDATE ON render_jobs FOR EACH ROW EXECUTE FUNCTION set_updated_at();
-- EVENTS
CREATE TABLE events (
  id SERIAL PRIMARY KEY,
...
CREATE TRIGGER trg_events_updated BEFORE
UPDATE ON events FOR EACH ROW EXECUTE FUNCTION set_updated_at();
-- SOFT DELETES: Add deleted_at to all tables
